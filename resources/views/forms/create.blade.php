                                ...
                                @for ($i = 0; $i < 30; $i++)
                                    <tr class="field-row border-b border-gray-200 dark:border-gray-700">
                                        <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">
                                            {{ $i + 1 }}</td>
                                        <td class="px-4 py-3">
                                            <textarea name="fields[{{ $i }}][description]" rows="1" placeholder="Enter description"
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" name="fields[{{ $i }}][quantity]"
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" name="fields[{{ $i }}][length]"
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" name="fields[{{ $i }}][width]"
                                                step="0.01" min="0" placeholder="0"
                                                oninput="calculateProduct(this)"
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" name="fields[{{ $i }}][height]"
                                                step="0.01" min="0" placeholder="0"
                                                oninput="calculateProduct(this)"
                                        ...
                                        <td class="px-4 py-3">
                                            <input type="number" name="fields[{{ $i }}][product]"
                                                step="0.01" min="0" placeholder="0" readonly
        ...
        <div
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
                <div class="flex items-center justify-center space-x-4">
                    ...
                    <button type="submit" form="formForm"
    ...
    <script>
        ...
        $(document).ready(function() {
            $('#formForm').on('submit', function(e) {
                ...
                $('.error-highlight').removeClass('error-highlight border-red-500');
                ...
                if (!clientName) {
                    ...
                    $('#client_name').addClass('error-highlight border-red-500');
                }
                ...
                if (!projectName) {
                    ...
                    $('#project_name').addClass('error-highlight border-red-500');
                }
                ...
                $('tbody#fieldsTableBody tr').each(function(index) {
                    const row = $(this);
                    const description = row.find('.field-description').val().trim();
                    const quantity = row.find('.field-quantity').val().trim();
                    const length = row.find('.field-length').val().trim();
                    const width = row.find('.field-width').val().trim();
                    const height = row.find('.field-height').val().trim();
                    const product = row.find('.field-product').val().trim();

                    // Check if row has any data
                    const hasAnyData = description || quantity || length || width || height ||
                        product;

                    if (hasAnyData) {
                        // If row has any data, all fields must be filled
                        if (!description || !quantity || !length || !width || !height || !product) {
                            incompleteRows.push(index + 1);
                            row.find('input, textarea').addClass('error-highlight border-red-500');
                        } else {
                ...
                if (incompleteRows.length > 0) {
                    errors.push(
                        `Row(s) ${incompleteRows.join(', ')} have incomplete data. All fields must be filled if any field has data.`
                    );
                }

                if (!hasAtLeastOneCompleteRow) {
                    errors.push('At least one complete field row is required.');
                }
